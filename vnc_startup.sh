...
function start_audio_out_websocket (){
	if [[ ${KASM_SVC_AUDIO:-1} == 1 ]]; then
		...
		$STARTUPDIR/jsmpeg/kasm_audio_out-linux kasmaudio 8081 4901 ${HOME}/.vnc/self.pem ${HOME}/.vnc/self.pem "firefox:$VNC_PW"  &
...
function start_audio_in (){
	if [[ ${KASM_SVC_AUDIO_INPUT:-1} == 1 ]]; then
		...
		$STARTUPDIR/audio_input/kasm_audio_input_server --ssl --auth-token "firefox:$VNC_PW" --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
...
function start_upload (){
	if [[ ${KASM_SVC_UPLOADS:-1} == 1 ]]; then
		...
		$STARTUPDIR/upload_server/kasm_upload_server --ssl --auth-token "firefox:$VNC_PW" &
...
function start_gamepad (){
	if [[ ${KASM_SVC_GAMEPAD:-1} == 1 ]]; then
		...
		$STARTUPDIR/gamepad/kasm_gamepad_server --ssl --auth-token "firefox:$VNC_PW" --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
